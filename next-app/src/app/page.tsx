                    ...
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {displayProducts.map((p, i) => {
                          ...
                          const priceOk = p.price < priceThreshold;
                          const colorOk = !color || p.color?.toLowerCase() === color.toLowerCase();
                          const nameOk = !clean || p.name?.toLowerCase().includes(clean.toLowerCase());
                          const isRelevant = priceOk && colorOk && nameOk;
                            ...
                            >
                              ...
                              {!smartMode && !isRelevant && (
                                <div className="mt-2 text-[10px] text-red-400/70 flex items-center gap-1">
                                  <span>&#x26A0;</span> {[!priceOk && "Over budget", !colorOk && "Wrong color", !nameOk && "Wrong product"].filter(Boolean).join(" + ")}
                                </div>
                              )}
                            </div>
                          );
